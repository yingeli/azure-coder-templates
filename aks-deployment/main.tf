...
data "coder_parameter" "allow_spot" {
  name         = "allow_spot"
  display_name = "Allow running on Azure Spot instances"
  description  = "Set this to true if allowing to run on Azure Spot instances"
  type         = "bool"
  default      = false
  mutable      = true
}
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        dynamic "toleration" {
          for_each = data.coder_parameter.nvidia_gpu.value > 0 ? [1] : []
          content {
            key      = "sku"
            operator = "Equal"
            value    = "gpu"
        ...
        dynamic "toleration" {
          for_each = data.coder_parameter.allow_spot.value ? [1] : []
          content {
